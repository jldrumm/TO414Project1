...
cluster1_norm_df <- as.data.frame(model.matrix(~.-1,cluster1_norm))
...
ANN_func <- function(cluster_norm) {
  ...
  func_model <- neuralnet(formula = yyes~., data = cluster_train, hidden = 1)
  
  func_prediction <- predict(func_model, newdata = cluster_test, type = "response")
  func_prediction <- ifelse(func_prediction < 0.5, 0, 1)
  ret_val <- confusionMatrix(as.factor(cluster_test$yyes), as.factor(func_prediction))
  
  return(ret_val)
}

result <- ANN_func(cluster1_norm)
```
## Logistics Model Function
```{r}
LM_func <- function(cluster_norm) {
  ret_val <- glm(yyes ~ ., data = cluster_norm, family = "binomial")
  return(ret_val)
}

lm_result <- LM_func(cluster1_norm)
```

## Debugging
```{r}
cluster_norm <- cluster1_norm
cluster_reg <- tele_norm[tele_norm$cluster == 1,]
  ...
  cluster_train_labels <- cluster_reg[-cluster_test, "yyes"]
  cluster_test_labels <- cluster_reg[cluster_test, "yyes"]
